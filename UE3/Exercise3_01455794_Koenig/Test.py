...
def adjust_datetime(date, time):
    """Adjusts the time and potentially the date if time is '2400', then rounds the time to the nearest hour."""
    if pd.isna(time):
...
flight_data['WHEELS_OFF'] = flight_data['WHEELS_OFF'].apply(clean_time)
flight_data['WHEELS_ON'] = flight_data['WHEELS_ON'].apply(clean_time)
flight_data[['FL_DATE', 'WHEELS_OFF']] = flight_data.apply(lambda row: adjust_datetime(row['FL_DATE'], row['WHEELS_OFF']), axis=1, result_type='expand')
flight_data[['FL_DATE', 'WHEELS_ON']] = flight_data.apply(lambda row: adjust_datetime(row['FL_DATE'], row['WHEELS_ON']), axis=1, result_type='expand')

# Conversion to datetime
flight_data['datetime_dep'] = pd.to_datetime(flight_data['FL_DATE'] + ' ' + flight_data['WHEELS_OFF'], errors='coerce')
flight_data['datetime_arr'] = pd.to_datetime(flight_data['FL_DATE'] + ' ' + flight_data['WHEELS_ON'], errors='coerce')
...
flight_data = flight_data.merge(
    weather_data,
    left_on=['ORIGIN', 'datetime_dep'],
    right_on=['station', 'valid'],
...
flight_data.to_csv('merged_data_exact_time.csv', index=False)
